...
export class ChannelsService {
  constructor(
    ...
    private channelChantsRepository: Repository<ChannelChats>,
  ...
  async getWorkspaceChannelChats(
    ...
    return this.channelChantsRepository
  ...
  async getChannelUnreadsCount(url: string, name: string, after) {
    const channel = await this.channelsRepository
      .createQueryBuilder('channel')
      .innerJoin('channel.Workspace', 'workspace', 'worksapce.url = :url', {
        url,
      })
      .where('channel.name = :name', { name })
      .getOne();

    return this.channelChantsRepository.count({
      where: {
  ...
  async postChat({ url, name, content, myId }) {
    ...
    const savedChat = await this.channelChantsRepository.save(chats);
    // savedChat.Channel = channel;
    // savedChat.User = user;
    const chatWithUser = await this.channelChantsRepository.findOne({
